...
**Timeout behavior**: The `--timeout` on `group-wait` is a _group-level_ deadline — the maximum wall-clock time to wait for all tasks in the group. Individual tasks have their own per-task timeout (passed via `task enqueue --timeout N`). A per-task timeout of 600s with a group timeout of 600s means all tasks get 600s each; the group waits up to 600s for the slowest. This is independent of `LockTTL` (2100s) since the lock is not held during parallel phases.
...
- **Lock TTL (2100s)**: Unchanged for sequential tasks. During parallel phases, the lock is not held — the gate-check releases it immediately.
