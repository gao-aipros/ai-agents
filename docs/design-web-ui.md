...
5. If the subprocess doesn't complete within `REQUEST_TIMEOUT` (default 2.5 hours), the handler sends SIGTERM, waits 10s, then SIGKILL. It writes an error message:
   ```json
...
POST /api/requests {thread_id?, repo?, request}
  ...
  handler (Go, in cmd/webui)
       ...
       │      (TTL = REQUEST_TIMEOUT + 5 min, i.e. 155 min — provides a 5-min margin
...
|---------|---------|---------|
| `REQUEST_TIMEOUT` | `9000` | Seconds before killing a stuck `claude -p` (2.5h) |
